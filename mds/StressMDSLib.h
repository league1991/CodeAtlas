...
namespace StressMDSLib
{
	...
	**********************************************/
	...
	public:
		...
		Matrix(int rows, int columns, T* value);			// initialization with col-major value arrays
		...
		T& getRef(int r, int c); // row, column
	...
	**********************************************/
	...
	template<class T>
	Matrix<T>::Matrix(int rows, int columns, T* valueArray) // initialization with value array
	{
		if(rows<1||columns<1)
		{
			printf("Invalid construction arguments: rows=%d, columns=%d\n",rows,columns);
	...
	template<class T>
	T& Matrix<T>::getRef(int r, int c) // row, column
	{
		return v[r][c];
	}
	...
	**********************************************/	
	...
	Matrix<double> * MDS_UCF(Matrix<double> * D, Matrix<double> * X0,int dim, int iter, double * weight = NULL, double distWeightPwr = 0.0);
