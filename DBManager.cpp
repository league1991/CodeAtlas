...
static DBManager *managerInstance = 0;
    ...
    d(new ManagerPrivate())
{
	...
	d->db.moveToNewThread(d->dbThread);
    d->dbThread.start();
...
void DBManager::fetchMore(QStandardItem *item, bool skipRoot)
{
...
//            SLOT(onWidgetIsCreated()), Qt::QueuedConnection);
    ...
    connect(this, SIGNAL(requestDocumentUpdated(CPlusPlus::Document::Ptr)),
            &d->db, SLOT(updateDocumentCache(CPlusPlus::Document::Ptr)), Qt::QueuedConnection);
...
//            this, SLOT(onTreeDataUpdate(QSharedPointer<QStandardItem>)), Qt::QueuedConnection);
    ...
    connect(this, SIGNAL(requestTreeDataUpdate()),
            &d->db, SLOT(updateDB()), Qt::QueuedConnection);

    // full reset request to parser
    connect(this, SIGNAL(requestResetCurrentState()),
            &d->db, SLOT(resetDataToCurrentState()), Qt::QueuedConnection);

    // clear cache request
    connect(this, SIGNAL(requestClearCache()),
            &d->db, SLOT(clearCache()), Qt::QueuedConnection);

    // clear full cache request
    connect(this, SIGNAL(requestClearCacheAll()),
            &d->db, SLOT(clearCacheAll()), Qt::QueuedConnection);
...
//            &d->parser, SLOT(setFlatMode(bool)), Qt::QueuedConnection);
    ...
    connect(codeModelManager, SIGNAL(aboutToRemoveFiles(QStringList)),
            &d->db, SLOT(removeFiles(QStringList)), Qt::QueuedConnection);

	// my signals
	connect(&d->db, SIGNAL(dbUpdated()), this, SIGNAL(dbUpdated()));

	// cursor update signal
	Core::EditorManager* editorManager = dynamic_cast<Core::EditorManager*>(Core::EditorManager::instance());
	if (editorManager)
	{
		connect(editorManager, SIGNAL(editorCreated(Core::IEditor *, const QString &)),
			this, SLOT(connectCursorUpdateSignal(Core::IEditor *, const QString &)));
	}
...
void DBManager::setState(bool state)
{
...
void DBManager::onWidgetIsCreated()
{
...
void DBManager::onWidgetVisibilityIsChanged(bool visibility)
{
...
void DBManager::onStateChanged(bool state)
{
    if (state) {
...
void DBManager::onProjectListChanged()
{
...
void DBManager::onTaskStarted(Core::Id type)
{
...
void DBManager::onAllTasksFinished(Core::Id type)
{
    if (type != CppTools::Constants::TASK_INDEX)
        return;
...
void DBManager::onDocumentUpdated(CPlusPlus::Document::Ptr doc)
{
    // do nothing if Manager is disabled
    if (!state())
        return;
...
void DBManager::connectCursorUpdateSignal(Core::IEditor *editor, const QString &fileName)
{
	// only connect cpp editors
#ifdef USING_QT_4
	...
	if (!mgr)
		return;
#endif
	if(!mgr->instance()->isCppEditor(editor))
		return;

	typedef TextEditor::BaseTextEditor TxtEditor;
	typedef TextEditor::BaseTextEditorWidget TxtEditorWidget;
	TxtEditor* txtEditor = dynamic_cast<TxtEditor*>(editor);
	if (!txtEditor)
		return;
	
	TxtEditorWidget* txtEditorWidget = txtEditor->editorWidget();
	if (!txtEditorWidget)
		return;	

	connect(txtEditorWidget, SIGNAL(cursorPositionChanged()), 
		&d->db, SLOT(updateCursor()), Qt::UniqueConnection);
}
...
void DBManager::gotoLocation(const QString &fileName, int line, int column)
{
...
void DBManager::gotoLocations(const QList<QVariant> &list)
{
	QSet<SymbolLocation> locations = AtlasUtils::roleToLocations(list);
	gotoLocations(locations);
}
void DBManager::gotoLocations(const QSet<SymbolLocation>& locations)
{
...
void DBManager::onRequestTreeDataUpdate()
{
    // do nothing if Manager is disabled
    if (!state())
        return;
...
void DBManager::setFlatMode(bool flat)
{
...
void DBManager::onTreeDataUpdate(QSharedPointer<QStandardItem> result)
{
    // do nothing if Manager is disabled
    if (!state())
        return;
...
void CodeAtlas::DBManager::setEnable( bool isEnable )
{
	emit setState(isEnable);
}
